...
# stacked['actual'] , stacked['eNet_pred'], stacked['ridge_pred'] = np.log(y), eNet_pred, ridge_pred
# ystacked = stacked.actual.astype(float)
# xstacked= stacked.loc[:, stacked.columns != 'actual'].astype(float)
# lm = linear_model.LinearRegression()
# lm.fit(xstacked, ystacked)
# list(zip(lm.coef_,xstacked.columns))
# =============================================================================
#.....STACKING CONT
...
model_stack = StackingRegressor(
    regressors=[eNet,rf,ridge,lasso,gbr,svr],
...
#prediction(stacked) df for kaggle submission
# =============================================================================
# def rmsle(y, y_pred):
...
subm3.drop('SalePrice', axis=1, inplace=True)
test_df = test_df1.loc[:, test_df1.columns != 'SalePrice'].astype(float)
stacked_pred = pd.Series(averaged_models.predict(test_df))
subm3['SalePrice'] = stacked_pred.apply(lambda price: exp(price))
subm3.to_csv('subm3.csv',index=False)   
########################################################################   
    

# =============================================================================
# eNet_pred = pd.Series(eNet.predict(test_df))
# 
# xstacked.isnull().sum()
# =============================================================================
